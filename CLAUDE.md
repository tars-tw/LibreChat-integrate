...
### TARS SQL Agent (LibreChat → pwc_tars, exposed as an MCP server)

pwc_tars's SQL agent is available in the chat tools menu as a loopback MCP server named **`sql_agent`** (title 資料庫查詢), injected into `mcpConfig` whenever `TARS_AUTH_URL` is set (`TARS_SQL_AGENT_ENABLED=false` opts out). `POST /api/tars/sql-agent` hosts a stateless streamable-http MCP server with two **static** tools — `list_databases` and `query_database` (question + `knowledge_base_id` → `POST /api/langflow-service/sql`). Both are scoped to the **active 專用腦**, mirroring pwc_tars's own chat path (`message/routes.py` resolves a database from the domain's `knowledge_base_ids`, not from everything the user can see): the candidate set is `has_sql_database` on `GET /api/knowledge_base/prepare_data` intersected with the domain's knowledge bases, and `query_database` authorizes against that same set. `injectTarsDomain` additionally names those databases in the system prompt when the turn enabled the SQL agent, so a single-database brain needs no `list_databases` round trip at all. pwc_tars owns the whole text-to-SQL loop (schema prompt from the KB↔database binding's `llm_table_info`, read-only guard, row formatting); LibreChat only authorizes the knowledge base (fail-closed for unlinked accounts) and relays the answer, so the result — a markdown table plus the SQL used — flows back into the normal agent loop and composes with every other tool. Because the tools are static, `tools/list` never depends on pwc_tars being up and nothing is fetched at boot.

`query_database` costs **two** LLM calls: the chat model decides to call the tool, then pwc_tars's nested loop writes and runs the SQL. The nested loop uses the **same model the chat turn is on** — `recordTarsChatContext` (mounted after `buildEndpointOption`, so the resolved agent names the model even on saved-agent turns whose payload omits `model`) records `userId → {model, domainId}` in a 15-minute per-process store (`packages/api/src/tars/chat.ts`), and the SQL client recalls it via the `X-Tars-User-Id` header. That same store is what carries the active brain to the MCP tools, which have no request context of their own. The model is matched against pwc_tars `model_profile` names — the same whitelist `ModelSelectorContext` filters the picker by, so the picker can only produce a match — and unmatched models (saved agents and assistants bypass that filter; it also fails open while pwc_tars is down) fall back to pwc_tars's default sys_model rather than letting pwc_tars 400 the call. `TARS_SQL_AGENT_MODEL` pins one model regardless. The side channel exists because `{{LIBRECHAT_BODY_MODEL}}` is not usable: a referenced body placeholder becomes mandatory and `compactAgentsBaseSchema` omits `model`, so saved-agent turns would take the whole MCP server down.

Route auth reuses the MCP gateway key (`X-Tars-Gateway-Key`) and `X-Tars-User-Id: {{LIBRECHAT_USER_ID}}` scoping; the pwc_tars service key is the `KEY_LANGFLOW_API_KEY` sys_config row (override: `TARS_SQL_SERVICE_KEY`). In the UI it is **not** presented as an MCP server: `useTarsSqlAgent` lifts its toggle into a top-level row of the tools menu (「資料庫查詢」, `Database` icon) beside web search and code interpreter, with its own composer badge (`SqlAgent.tsx`) and pin, and `MCPSubMenu` / `MCPSelect` filter it out so it is never offered twice. Selection state is still plain `ephemeralAgent.mcp` membership, so nothing server-side changes — and the entry must stay in `selectableServers`, because `useMCPSelect` prunes `mcpValues` of anything not in that list. The entry name deliberately sits **outside** the `tars_` namespace — it proxies a pwc_tars capability, not an `mcp_server` row, so it has no per-user opt-in; `useMCPServerManager` hides it from non-tars accounts instead (shared constant `TARS_SQL_MCP_SERVER_NAME` in data-provider). Other env: `TARS_SQL_AGENT_USE_GATEWAY` (route the SQL agent's own LLM back through LibreChat's gateway), `TARS_SQL_AGENT_TIMEOUT_MS`, `TARS_SQL_AGENT_TITLE` / `_DESCRIPTION` / `_SELF_URL`. Key files: `packages/api/src/tars/sql/{client,server,config}.ts`, `packages/api/src/tars/chat.ts`, `api/server/middleware/recordTarsChatContext.js`, route in `api/server/routes/tars/mcp.js`, injection in `api/server/services/Config/app.js`. **No pwc_tars-side change is required** — `/api/langflow-service/sql` already exists for Langflow.
...
Key files: `packages/api/src/tars/{memory,langflow}/`, registry entries in `packages/api/src/tools/registry/definitions.ts`, dispatch in `handleTools.js`, gates in `ToolService.js`. **No pwc_tars-side change was required.**
