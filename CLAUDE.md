...
- **Endpoint**: `POST /api/agents/v1m/chat/completions` (`v1m` = model passthrough, distinct from the agent endpoint `/api/agents/v1` where `model` is an agent_id). Here `model` is a **real `<provider>/<model>`** pair, e.g. `openAI/gpt-5.4-mini`, `anthropic/claude-...`, `google/gemini-...` (LibreChat has no model→provider reverse lookup, so the prefix is required). Standard OpenAI chat-completions body (`messages`, `stream`, `temperature`, ...). No agent pipeline — a bare ephemeral agent (no LibreChat-side tools, no system prompt).
- **Client-side tools**: the caller's own `tools` are supported and are what makes pwc_tars's tool-calling agents (`agent_core.runner`) work over the gateway. They are bound to the model as **schema-only definitions** (`toolDefinitions` → the SDK's event-driven ToolNode), so LibreChat never executes anything: the moment the model calls one, the `ON_TOOL_EXECUTE` dispatch is intercepted, the run is stopped, and the calls come back as `tool_calls` with `finish_reason: 'tool_calls'` for the caller to run and return as `role: 'tool'` messages. `tool_choice` accepts only `'auto'`/`'none'` (the graph binds tools without a forcing option, so `'required'`/named choices are rejected rather than silently downgraded), and caller tools are refused for a target agent that has tools of its own — a non-empty `toolDefinitions` would switch that agent's own tools to the same event-driven dispatch. Caller tool schemas go through the same content-protection inspection as the agent's own definitions. A provider failure after the stream has started is written as OpenAI's `{"error": {...}}` SSE object (then `[DONE]`), never as assistant text: every OpenAI SDK raises on that object, which is what lets pwc_tars's rate-limit handling (`is_rate_limit_error` on the message) see a 429 instead of recording the error sentence as a table task's answer.
- **How a caller invokes it** (e.g. a plain OpenAI client / LangChain `ChatOpenAI`): point `base_url` at `http://<host>:3080/api/agents/v1m` and set `model` to `<provider>/<model>`. Auth (LibreChat `.env`, `# LLM Gateway` block): `LLM_GATEWAY_ALLOW_UNAUTHENTICATED=true` (closed internal net; no key), OR `LLM_GATEWAY_SERVICE_KEY=<secret>` sent as `Authorization: Bearer <secret>`, else per-user remote-agent API keys. The gateway has its own JSON body cap, `LLM_GATEWAY_BODY_LIMIT` (default `50mb`, wired ahead of the app-wide 3mb parser in `api/server/index.js`): pwc_tars sends whole prompts through it, and a table task's map batch alone carries a knowledge base's worth of documents.
...
- **Setup**: `docs/TARS_INTEGRATION.md` §1.3 covers the `.env` values (`LLM_GATEWAY_*`, `HOST`); the gateway itself is documented here.

### TARS MCP Gateway (pwc_tars MCP → LibreChat; pwc_tars is the sole MCP management surface)

pwc_tars's MCP `openapi` (Swagger import), `custom_api`, and `external` (real MCP servers over stdio / streamable_http, executed by pwc_tars) servers surface in LibreChat's MCP tool system via **per-server loopback MCP servers**: `POST /api/tars/mcp/:serverId` hosts a stateless streamable-http MCP server (real `@modelcontextprotocol/sdk` server) whose `tools/list`/`tools/call` proxy to pwc_tars's `/api/mcp` REST API — pwc_tars stays the source of truth (definitions, domain permissions, per-user toggles/credentials, `mcp_logs` audit); only `builtin` is not proxied. When `TARS_AUTH_URL` is set, `withTarsMcpConfig` (now async) fetches the pwc_tars server list at base-config load and injects **one `mcpConfig` entry per enabled server, named `tars_<code>`** (helper `tarsMcpServerName`/`isTarsMcpServerName` in data-provider; name is a `normalizeServerName` fixed point), so each server is its own toggle in the chat dropdown/agent tools and LibreChat's entire MCP pipeline works with zero changes. pwc_tars down at boot → nothing injected + one-shot 60s retry (`tarsMcpInjectionFailed`); every LibreChat-proxied admin mutation (create/update/delete/sync, tool toggle, domain save) calls `invalidateTarsMcpToolsCache()` + `invalidateConfigCaches()` so entries refresh immediately. The legacy aggregate `POST /api/tars/mcp` (single `tars` entry, `<code>__<tool>` names) remains for YAML-pinned setups (deprecated). Per-user scoping via `X-Tars-User-Id: {{LIBRECHAT_USER_ID}}` header → `User.tarsId` → pwc_tars `GET /available-tools?user_id=` (full permission stack pwc_tars-side: domain grants, `mcp_tool_ids` whitelists, `sys_user_mcp` opt-in toggles; admin role_id=1 bypasses domain whitelists but own toggles still apply); unlinked users fail closed. Per-server tool names are **unprefixed** (final: `<tool>_mcp_tars_<code>`). The gateway lists every tool pwc_tars grants; the cap is applied **per turn** in `initializeAgent` by `selectRelevantTools` (`packages/api/src/agents/relevance.ts`, a port of pwc_tars `select_relevant_tools`): when the non-deferred MCP definitions across all servers (`tars_*` and langflow alike) exceed `MCP_MAX_TOOLS` (default 120, pwc_tars `SysConst.MCP_MAX_TOOLS`), they are ranked against the user message (`req.body.text`; IDF-weighted word overlap with CJK bigrams, load order when there is no message) and the least relevant overflow is dropped from both `toolDefinitions` and `toolRegistry` with a warning naming each dropped tool — native tools, actions and `defer_loading` tools never compete for the cap. Route auth is a gateway key (HMAC of `JWT_SECRET`, override `TARS_MCP_GATEWAY_KEY`), not JWT. pwc_tars gates its MCP mutations (server create/update/delete/test/sync, parse-openapi, tool edits, domain bindings) behind `@admin_required`, so the admin proxy calls them with `tarsFetch({ asUser: req.user.tarsId })`, which signs a 60s HS256 `{user_id}` bearer with `TARS_JWT_SECRET` (= pwc_tars `SysConst.DEFAULT_SECRET_KEY`; unset → those calls fail 503-before-send). custom_api / openapi tool fields may carry a fixed `value` (tool editor 「固定值」 column, variable list from `GET /api/tars/mcp/admin/system-variables`) with `{{TARS_CURRENT_LOGINID}}` / `{{TARS_CURRENT_USER_EMAIL}}` / `{{TARS_CURRENT_USER_ID}}`; pwc_tars hides bound fields from the tool schema and substitutes them per call from `/api/mcp/execute`'s `user_id`, so the gateway needs no change for it. **When TARS is enabled (`startupConfig.tarsMcpEnabled`), the native MCP management UI is hidden** (side-panel add/list, agent-marketplace "create MCP") — librechat.yaml servers (langflow) keep working — and the chat dropdown filters `tars_*` entries to those the user enabled (`useMCPServerManager` + `useTarsMcpUserSettingsQuery`; enforcement stays server-side). `/mcp-settings` is the **single MCP surface**, open to any pwc_tars-linked account (guard is `user.provider === 'tars'`, not admin — every admin endpoint is gated server-side by `requireTarsAdmin`, and the admin server-list query is disabled for non-admins so it is never requested). Tabs: **My Tools** (the user's own catalog — per-server/per-tool toggles and credentials, `/api/tars/mcp/user/*`; the only tab a non-admin sees, and their default), plus admin-only **Servers** (CRUD for all three types incl. external transport editor stdio/streamable_http, priority/tags, per-tool enable via `PUT /api/tars/mcp/admin/tools/:toolId`, parse-openapi, test/sync), **Permissions** (domain↔server→tool whitelist tree → `POST /api/tars/mcp/admin/domains/save`, full-overwrite), **Logs** (`GET /api/tars/mcp/admin/logs`). `?tab=` deep-links a tab; unknown values (or admin tabs requested by a non-admin) fall back to the account's default (admin → Servers, user → My Tools). The MCP side panel's `TarsCard` is the one entry point (`/mcp-settings?tab=mytools`); it has no gear and there is no longer a "TARS Tools" dialog. **Per-user credentials are also reachable from chat**: `MCPSelect` / `MCPSubMenu` render a key badge (amber = required, green = set) on any `tars_*` entry whose server declares `requires_user_credentials`, opening `McpCredentialsDialog` from `ToolDialogs` (the badge row's single dialog host — the menus unmount on close). Both surfaces share `McpCredentialsForm` and read state from the same `useTarsMcpUserSettingsQuery` cache (exposed through `useTarsMcpTools`, so no extra request), and credential save/clear invalidates `tarsMcpUserSettings` + `tarsMcpDomainTools` since pwc_tars re-syncs a server's tools once it authenticates. Frontend: `client/src/components/McpSettings/` (`McpSettingsView`, `McpUserToolsTab`, `McpServerModal`, `McpServerToolsPanel`, `McpPermissionsTab`, `McpLogsTab`), `client/src/components/Tars/{McpCredentialsForm,McpCredentialsDialog}.tsx`, `client/src/hooks/MCP/useTarsMcpTools.ts`, hooks in `client/src/data-provider/Tars/`. Key files: `packages/api/src/tars/mcp/{client,server,config,admin,user}.ts`, `api/server/routes/tars/mcp.js`, injection in `api/server/services/Config/app.js`. Setup needs nothing beyond `TARS_AUTH_URL` (`docs/TARS_INTEGRATION.md` §1.3); this section is the reference.
...
### TARS RAG Agent (LibreChat → pwc_tars, a native LibreChat tool)

pwc_tars's RAG (knowledge-base retrieval) is a **first-class LibreChat tool** at the same level as `sql_agent`: `Tools.rag_agent` / `AgentCapabilities.rag_agent` / `PermissionTypes.RAG_AGENT` / `interface.ragAgent` / `ephemeralAgent.rag_agent`, constructed in `handleTools.js` beside the SQL agent, shown in the chat tools menu as 「知識庫檢索」 (`BookOpen` icon) with its own composer badge and pin. Available whenever `TARS_AUTH_URL` is set and `rag_agent` is listed in librechat.yaml capabilities.

One tool, `rag_agent({ question, knowledge_base_ids? })` → `POST /api/langflow-service/rag`. Its searchable knowledge bases are the ones the active 專用腦 binds — the exact set pwc_tars's own chat retrieves from — resolved per request by `listTarsScopedKnowledgeBases` (`packages/api/src/tars/scope.ts`, the shared brain-scoped listing `sql_agent` now filters `has_sql_database` on top of) and written into the tool's description plus the `rag_agent` runtime context. Empty `knowledge_base_ids` searches all of them; an id outside the scope refuses the whole call rather than silently narrowing it; unlinked accounts fail closed. pwc_tars owns retrieval, reranking and the cited answer (source file names inline), and the nested run's trace comes back as the same `tars_trace` attachment the other capability tools attach. Model matching and the gateway headers are the same as `sql_agent`; env `TARS_RAG_AGENT_TIMEOUT_MS` (default 240s). Key files: `packages/api/src/tars/rag/{client,tool}.ts`, `packages/api/src/tars/scope.ts`, UI `client/src/components/Chat/Input/RagAgent.tsx` + `ToolsDropdown.tsx`. **No pwc_tars-side change was required** — `/api/langflow-service/rag` already existed for Langflow.

### TARS Agent (`tars_agent` — pwc_tars's own loop, one nested run per turn)

The five loop-backed TARS capabilities (`sql_agent`, `rag_agent`, `chart_agent`, `data_query`, `table_task`) each ran a whole pwc_tars agent loop per call, so a question that touched two of them cost two nested runs side by side. The chat now mounts **one** `Tools.tars_agent` whenever any of them applies, and that tool calls `POST /api/langflow-service/agent` (pwc_tars `capability_runner.run_agent`, the one pwc_tars-side change of this feature): a single `AgentRequest` bound the way pwc_tars's own chat is — `knowledge_search` for the brain's knowledge bases, `sql_*` for the bound database, `data_*` (+ `table_task` once knowledge bases are bound too) for the conversation's spreadsheets, and the chart / file tools pwc_tars always carries — so **pwc_tars's model decides per question which tools it needs**, with pwc_tars's own tool guide, and the trace shows what it chose. `tars_agent({ question, knowledge_base_ids?, database_knowledge_base_id?, document_ids? })`: the optional arguments only narrow; the only database is bound automatically, several databases mean the model names one (or none, pwc_tars's own "several bound → no SQL" behaviour).

What a turn binds is `resolveTarsAgentBindings` (`packages/api/src/tars/agent/bindings.ts`): the chat switches 資料庫查詢 / 知識庫檢索 / 產生圖表 (each ANDed with its librechat.yaml capability, so the combined tool never reaches what the individual tool would have been filtered for) plus the memory snapshot's structured files. `load.ts` / `added.ts` push `tars_agent` for the switches, `initialize.ts` adds it for spreadsheets (in place of the old `data_query` + `table_task` auto-equip), and the same resolver runs in `handleTools.js` (tool construction) and `ToolService.js` (runtime context); the tool's own gate is `isTarsConfigured()`. Timeout `TARS_AGENT_TIMEOUT_MS` (default 240s), stretched to `TARS_TABLE_TASK_TIMEOUT_MS` when spreadsheets and knowledge bases are both bound. The individual tools stay for saved agents that list them explicitly and for Langflow. Plugin tools (`tars_plugin_*`) and `tars_*` MCP servers are direct single calls, not loops, and keep their own pipelines. Key files: `packages/api/src/tars/agent/{bindings,client,tool}.ts`; pwc_tars `run_agent` + route `/agent` + the Langflow `TarsAgent` component's `agent` mode.
...
- **`Tools.chart_agent`** (「產生圖表」) — full sql_agent-style wiring: `AgentCapabilities.chart_agent` (must be listed in librechat.yaml capabilities), `PermissionTypes.CHART_AGENT`, `interface.chartAgent`, `TEphemeralAgent.chart_agent`, badge `ChartAgent.tsx` + pin + ToolsDropdown (gated on `startupConfig.tarsAuth`). pwc_tars renders a PNG under its unauthenticated `/static`, on whatever host its sys_config `HOST` names (a dev pwc_tars sharing the deployed DB gets the deployed host, where the file does not exist). So `runLangflowCapability` rewrites every `/static/{quickchart,generate_output}` link in `answer`/`chart_url`/`file_url` to LibreChat's relay `GET /api/tars/static/:dir/*splat?sig=` (`packages/api/src/tars/assets.ts`, mounted ahead of `/api/tars` in `api/server/index.js`), and the browser never reaches pwc_tars. The relay serves only those two dirs and a fixed file-type list under its own content type (never `/static/uploads`), and authorizes by an HMAC of `JWT_SECRET` over the path rather than the session: an `<img>` cannot send the JWT, and shared views keep working. Links in messages from before the relay still point at pwc_tars.
- **`Tools.data_query` / `Tools.table_task`** — **never user-toggled**: when the snapshot has active structured docs, `initialize.ts` now equips `tars_agent` (which binds them as its spreadsheets, see the TARS Agent section) before `loadTools` (ephemeral AND saved agents); the two individual tools remain for saved agents that list them, the structured-file list goes in as `toolContextMap['tars_memory_data']`, and ToolService gates them on `isTarsConfigured()` only. Both send `document_ids` (defaulting to all attached; requested ids outside the snapshot are dropped). `table_task` additionally resolves `knowledge_base_ids` from the active 專用腦 (like `buildTarsSqlContext` but without the `has_sql_database` filter), refuses cleanly with no domain/no KBs, runs up to ~29min (`TARS_TABLE_TASK_TIMEOUT_MS` default 1_740_000 — watch reverse-proxy idle timeouts), and appends the xlsx download link.

- **Run trace** — every capability reply carries pwc_tars's `trace`: the same records its own chat persists under `extra_content.trace` (thinking, notes, each nested tool call with input/output, runtime notices, plan), produced pwc_tars-side by `capability_runner._run_agent` → `sse_adapter.collect_agent_trace` (the headless twin of `consume_agent_stream`). The four tools are `content_and_artifact` (`langflowToolResult` / `toTarsTraceArtifact` in `langflow/client.ts`): the model still reads only the answer, the trace rides as a `Tools.tars_trace` attachment (`TTarsTraceArtifact`, both tool-end callbacks in `callbacks.js`) and renders under the tool call as 「執行過程」 (`client/src/components/Chat/Messages/Content/TarsTrace.tsx`, routed by `isTarsCapabilityToolName` in `Part.tsx`; a port of pwc_tars's `AgentTracePanel`). An older pwc_tars simply sends no `trace`, so nothing is attached.

Key files: `packages/api/src/tars/{memory,langflow}/`, registry entries in `packages/api/src/tools/registry/definitions.ts`, dispatch in `handleTools.js`, gates in `ToolService.js`. The only pwc_tars-side change is the `trace` field on `/api/langflow-service/*` replies (pwc_tars branch `feature/langflow_service_trace`); everything else needed none.

### TARS Plugin Tools (`tars_tool_sdk` → LibreChat, native dynamic tools)

pwc_tars's plugin tools — Python `StandardTool` files dropped into `TARS_TOOL_PLUGINS_DIR` (`backend/tars_tool_sdk/README.md`) — surface in LibreChat as **native tools named `tars_plugin_<name>`** (`tarsPluginToolName` / `isTarsPluginToolName` / `tarsPluginNameFromToolName` in data-provider `src/tars.ts`), not as MCP servers. pwc_tars stays the source of truth for the whole lifecycle: discovery (`GET /api/domain_settings/plugin_tools`, admin `POST .../plugin_tools/reload`), the per-brain switches (`sys_domain.domain_functions` under `plugin:<name>` — `{kind:'plugin', name, enabled, default_value, display_name, description}`, the exact shape pwc_tars's own editor writes, so both UIs read each other's data), the input schema (`GET /api/langflow-service/tools`, `kind === 'plugin'`, the pydantic `input_schema` passed to `tool()` as JSON schema after `normalizeJsonSchema`), and execution (`POST /api/langflow-service/tools/<name>` with `inputs`, `context.model_name`, and `settings: {question, user_id, domain_id, plugin_tool_names}` mirroring what pwc_tars's chat hands a plugin as `ctx.settings`). Manifests are cached 5 min (`primeTarsPluginManifests`, dropped by an admin rescan) because that listing health-checks pwc_tars's model profiles.

Semantics mirror pwc_tars's chat exactly: the **brain's `domain_functions` is the gate** — `enabledTarsPluginFunctions(domain.domain_functions)` decides which plugins the chat offers and which start on, the user flips them per chat (`ephemeralAgent.tars_plugins: string[]`, hook `client/src/hooks/Tars/useTarsPluginTools.ts`, one Puzzle row per plugin in `ToolsDropdown` + composer badges `TarsPlugins.tsx`, pins in `PIN_TARS_PLUGINS_`), and the server re-checks in both ToolService paths via `resolveTarsPluginToolNames` (the port of pwc_tars `resolve_plugin_tool_names`: domain-enabled ∩ pwc_tars-loaded, fail-closed for unlinked users / no `domain_id` / pwc_tars errors). Definitions come from the primed cache through `getToolDefinition`'s fallback (`getTarsPluginDefinition`), instances from `handleTools.js` (`createTarsPluginTool`, `packages/api/src/tars/plugins/tool.ts`); a plugin that needs a model (`requires: ['llm']`) runs it on the chat model through the gateway like every other langflow-service call. **Admin surface** is the LibreChat 專用腦 editor (`Admin/Tars/Domains/Modal.tsx`, section 外掛工具 with 重新掃描 + 是否顯示 / 預設開啟 per plugin, invalid plugins listed but locked): the client sends `plugin_functions` and `packages/api/src/tars/domains.ts` merges it into the stored block server-side (`mergeTarsPluginFunctions`; on create pwc_tars defaults the block first, then a follow-up update merges), so the built-in feature keys pwc_tars manages are never rewritten from LibreChat. Env: `TARS_PLUGIN_TOOL_TIMEOUT_MS` (default 240s). **One pwc_tars-side change** rode along: `standard_tool_runner.run_standard_tool` now takes `ctx.question` from `settings.question` on the direct path, so plugins that read the user's message (`summarize_text` with `text` omitted) behave as in pwc_tars's own chat.
