...
### TARS MCP Gateway (pwc_tars OpenAPI / custom-API tools → LibreChat MCP)

pwc_tars's MCP `openapi` (Swagger import) and `custom_api` servers surface in LibreChat's MCP tool system via a **loopback MCP server**: `POST /api/tars/mcp` hosts a stateless streamable-http MCP server (real `@modelcontextprotocol/sdk` server) whose `tools/list`/`tools/call` proxy to pwc_tars's `/api/mcp` REST API — pwc_tars stays the source of truth (definitions, domain permissions, `mcp_logs` audit). When `TARS_AUTH_URL` is set, `withTarsMcpConfig` auto-injects a `tars` entry into `mcpConfig` at base-config load, so LibreChat's entire MCP pipeline (chat dropdown, agent tools, caching, permissions) works with zero changes. Per-user scoping via `X-Tars-User-Id: {{LIBRECHAT_USER_ID}}` header → `User.tarsId` → pwc_tars `GET /available-tools?user_id=` (full permission stack applied pwc_tars-side: domain grants, `mcp_tool_ids` tool whitelists, `sys_user_mcp` per-user toggles/overrides — servers are opt-in: OFF until the user enables them in the "TARS Tools" panel; admin role_id=1 bypasses domain whitelists but their own per-user toggles still apply); unlinked users fail closed to zero tools. `POST /execute` carries `user_id` too → visibility re-check (403), per-user credential merge (`sys_user_mcp.auth_credentials`), attributed `mcp_logs` audit (pwc_tars-side support added in its `routers/mcp/routes.py`: `resolve_user_visible_tool_map`, `resolve_credential_domain_id`). Route auth is a gateway key (HMAC of `JWT_SECRET`, override `TARS_MCP_GATEWAY_KEY`), not JWT. Tool names: `<serverCode>__<toolName>` (final: `..._mcp_tars`). LibreChat also ships its own UI on top: admin page `/mcp-settings` (create/edit openapi & custom_api servers, parse-openapi preview, test/sync — proxied via `/api/tars/mcp/admin/*`, `requireTarsAdmin`) and a per-user "TARS Tools" dialog (tool catalog with schemas, per-server/per-tool toggles → `sys_user_mcp`, verified credential entry — `/api/tars/mcp/user/*`, backed by pwc_tars `GET|PUT /api/mcp/user-settings[/credentials]`). Frontend: `client/src/components/McpSettings/`, `client/src/components/Tars/McpToolsDialog.tsx`, hooks in `client/src/data-provider/Tars/`. Key files: `packages/api/src/tars/mcp/{client,server,config}.ts`, `api/server/routes/tars/mcp.js`, injection in `api/server/services/Config/app.js`. Full docs: `docs/TARS_INTEGRATION.md` §6.6.
